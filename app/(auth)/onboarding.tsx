      ...
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.progreso}>
          {([1, 2, 3] as Paso[]).map((p) => (
            <View
              key={p}
              style={[styles.progresoPunto, paso >= p && styles.progresoPuntoActivo]}
            />
          ))}
        </View>
            ...
            <View style={styles.objetivosGrid}>
              {ObjetivosOptions.map((op) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.objetivoCard,
                    objetivo === op.value && styles.objetivoCardActivo,
                  ]}
                  onPress={() => setObjetivo(op.value as ObjetivoFitness)}
                >
                  <Text style={styles.objetivoEmoji}>{op.emoji}</Text>
                  <Text style={[
                    styles.objetivoLabel,
                    objetivo === op.value && styles.objetivoLabelActivo,
                  ]}>
                    {op.label}
                  </Text>
                </TouchableOpacity>
